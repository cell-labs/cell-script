...
func (c *Compiler) compileOperatorNode(v *parser.OperatorNode) value.Value {
	...
	case "string":
		if v.Operator == parser.OP_ADD {
			...
			c.contextBlock.NewCall(c.osFuncs.Strcpy.Value.(llvmValue.Named), backingArray, c.contextBlock.NewExtractValue(leftLLVM, 1))

			// Append right to backing array
			c.contextBlock.NewCall(c.osFuncs.Strcat.Value.(llvmValue.Named), backingArray, c.contextBlock.NewExtractValue(rightLLVM, 1))
