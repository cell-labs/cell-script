...
Args parse_args()
{
    ...
    int *owner_mode = &args.owner_mode;
...
//     CHECK2(!input6t->is_none(&input), ERROR_INVALID_MOL_FORMAT);
...
// int get_all_extension_scripts(Args *args, int owner_mode, uint8_t g_code_buff[], uint32_t g_code_used, uint8_t input_lock_script_hashes[], uint32_t input_lock_script_hash_count)
// {
//     int err = 0;
//     mol_seg_t raw_extension_seg = {0};
//     raw_extension_seg.ptr = args->var_data;
//     raw_extension_seg.size = args->var_len;
...
/// and check extension script valiated
// // If the extension script is identical to a lock script of one input cell in
// // current transaction, we consider the extension script to be already
// // validated, no additional check is needed for current extension
// int is_extension_script_validated(mol_seg_t extension_script,
